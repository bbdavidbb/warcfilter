...
const CCWebGraphCreator = require('./CCWebGraphCreator')
...
const watLimit = "watLimit"
const pairLimit = "pairLimit"
const pathOffset = "pathOffset"
...
class Main {
    ...
    constructor() {
        this.error = false;
        this.errorText = "";
        this.flags = [src, dest, mode, fileType, date, url,recordLimit, fileLimit, type, compressed, watLimit, pairLimit, pathOffset]
    }
    ...
    run = async (input) =>{
        ...
        let obj = await this.parseArgs(input)


        if(this.error) {
            console.log("\n ERROR arguments are improperly formatted: " + this.errorText + '\n')
            return;
        }
        ...
        else if(obj[mode] === "genCCWebGraph") {
            console.log("running CCWebgraphCreator...")
            await this.runCCGraphCreator(obj[src], obj[dest], obj)
        }
        else {
    ...
    formatArgs = async (obj) => {
        ...
        await this.checkExists(obj[src])
        await this.checkImproperArgs(obj)
        await this.rmIfExists(obj[dest])

        return obj;
    }

    checkExists = async (src) => {
        return new Promise( async (resolve, reject) => {
            fs.access(src[0], fs.constants.R_OK | fs.constants.W_OK, (err) => {
                if(err) {
                    this.error = true;
                    this.errorText += ' file in "src" argument does not exit; '
                    // file does not exists
                    resolve(true);
                }   
            });
    ...
    checkImproperArgs = async (obj) => {
        return new Promise( async (resolve, reject) => {
           for(let property in obj) {
               if(obj[property].includes(":")) {
                   this.error = true;
                   this.errorText += ' key word argument: ' + property + ' is either mispelled or does not exists'
                   resolve(true);
               }
           }
           resolve(true);
    ...
    runCCGraphCreator = async (origDir, destDir, args) => {
        let ccGraphCreator = new CCWebGraphCreator(origDir[0], destDir[0], args)
        let t0 = Date.now();
        await ccGraphCreator.generateGraphFile();
        let t1 = Date.now();
        let seconds = ((t1 - t0)) / 1000;
        seconds = Number((seconds).toFixed(6))
        let total = seconds + " seconds were needed to create the web graph file \n\n";
        console.log(total)
    }
}
